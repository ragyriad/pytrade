...
from .data.constants import SECURITY_UPDATE_FIELDS, SECURITY_UNIQUE_FIELD, ACCOUNTPOSITION_UNIQUE_FIELD, ACCOUNTPOSITION_UPDATE_FIELDS, ACTIVITY_UNIQUE_FIELD, ACTIVITY_UPDATE_FIELDS
...
def syncQestradeAccounts(request):
    ...
    try:
        ...
        accountObjToSave = [Account(type=account['type'],account_number=account['number'],status=account['status'],
                    is_primary=account['isPrimary'],
                    last_synced = datetime.now(),
                    currency= "CAD",
                    account_broker_id = "Questrade") for account in accounts ]
        Account.objects.bulk_create(accountObjToSave)
        return JsonResponse({"count": len(accounts), "Saved Accounts": accounts}, status=status.HTTP_200_OK)
    except Exception as e:
        print(e.args)
...
def syncQuestradeActivities (request):
    try:
        questradeObject = authentication()
        fetchedAccounts = AccountSerializer(Account.objects.filter(account_number__in = ["51978003", "51983522"]), many=True).data
        activitiesToSave = []
        securitiesToSave = []
        security_list = []
        for account in fetchedAccounts:
            ...
            for year in range(startingYear, 2024):
                for month in range(1,13):
                    ...
                    queryEndTime = datetime(year, month, lastDayOfMonth,tzinfo=timezone.utc)
                    print(account["type"] + " Account# " + 
                  account["account_number"] + " Request Period " + str(queryStartTime).split(' ')[0] + "-" + str(queryEndTime).split(' ')[0])
                    ...
                    for activity in activities:
                        ...
                        security_list = [getattr(security, 'id') for security  in securitiesToSave]
                        
                        if (activity['type'] == 'Trades' and 'symbolId' in activity and activity['symbolId'] not in security_list):
                            securitiesToSave.append(Security(
                                id= activity['symbolId'],
                                currency=activity['currency'],
                                symbol=activity['symbol'],
                                type= activityType if activityType == "Option" else "Equity"
                                ))
                            
                        activitiesToSave.append(Activity(
                            id=activity['id'] if 'id' in activity else uuid.uuid4().hex,
                            currency=activity['currency'],
                            ...
                            security_id= activity['symbolId'] if (activity['type'] == 'Trades' and 'symbolId' in activity) else None,
                            account_id=account_number
                            ))
        
        securityRecords = safeBulkCreate(Security, securitiesToSave,uniqueField=SECURITY_UNIQUE_FIELD, updateFields=SECURITY_UPDATE_FIELDS)
        securityRecords = SecuritySerializer(securityRecords,many=True).data
        activityRecords = safeBulkCreate(Activity, activitiesToSave, uniqueField=ACTIVITY_UNIQUE_FIELD, updateFields=ACTIVITY_UPDATE_FIELDS)
        activityRecords = ActivitySerializer(activityRecords, many=True).data
        return JsonResponse({"security_count": len(securityRecords),"security": securityRecords, "activity_count" : len(activityRecords), "activity": activityRecords}, status=status.HTTP_200_OK)
    except Exception as e:
        print(e.args)
