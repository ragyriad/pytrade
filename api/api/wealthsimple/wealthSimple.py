...
class wealthSimple:
    ...
    def wsimple_login(
        ...
        two_factor_callback: callable = None):
            ...
            print(f"Pre-login: {initial_request.status_code}/ {str(initial_request.content)}")
            ...
            if "x-wealthsimple-otp-required" in initial_request.headers:
                ...
                otp_headers = (
                    initial_request.headers["x-wealthsimple-otp"].replace(" ", "").split(";")
                )
                ...
                if final_request.status_code == 200:
                    if self.internally_manage_tokens:
                        self.session.headers["Authorization"] = f"Bearer {final_request.headers['X-Access-Token']}"
                        self.box = TokensBox(
                    ...
                    set_instance_cache(self, self.box.access_expires)
                    del final_request
                    return self
                else:
